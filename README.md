# Salut, moi c'est Mehdi 👋

Étudiant en **BUT Informatique** (3ᵉ année) et développeur orienté **Backend & Web**.  
J'aime concevoir des architectures solides, des API performantes et des interfaces claires et réactives.

---

### 🛠️ Stack technique

**Langages & Backend**  
<p align="left">
  <img src="https://skillicons.dev/icons?i=python,flask,django,js,dart,postgres,sqlite" alt="Backend and Languages" />
</p>

**Frontend & Mobile**  
<p align="left">
  <img src="https://skillicons.dev/icons?i=html,css,vue,flutter" alt="Frontend and Mobile" />
</p>

**DevOps, Environnement & Outils**  
<p align="left">
  <img src="https://skillicons.dev/icons?i=linux,docker,git,cloudflare,postman" alt="Tools and Environment" />
</p>

---

### 🚀 Ce que je fais en ce moment
- 💻 Conception et déploiement d'applications web fullstack (APIs REST, architectures modulaires).
- 📱 Développement d'applications mobiles cross-platform.
- 🐧 Toujours sous Linux (Fedora) pour le dev au quotidien.

---

### 📊 Statistiques GitHub

<p align="left">
  <img src="https://github-readme-stats.vercel.app/api/top-langs/?username=MehdiAkhh&layout=compact&theme=tokyonight&hide_border=true" alt="Top Langages" />
</p>

---

### 🌐 Me retrouver

[![Portfolio](https://img.shields.io/badge/Portfolio-mehdi--akhenkham.fr-blue?style=flat-square&logo=google-chrome&logoColor=white)](https://mehdi-akhenkham.fr)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Profil-0A66C2?style=flat-square&logo=linkedin&logoColor=white)](https://linkedin.com/in/TON_LINKEDIN)
[![Email](https://img.shields.io/badge/Email-Me_contacter-D14836?style=flat-square&logo=gmail&logoColor=white)](mailto:TON_EMAIL)
